...
dat1 <- dat0 %>%
  mutate(
    # BMI: 999 = insufficient data, -1 = improbable value
...
dat2 <- dat1 %>%
  mutate(
    logVLOAD    = ifelse(is.na(VLOAD), NA, log10(VLOAD + 1)),
...
# outcomes across all years + baseline-fixed exposure/covariates
# -----------------------------
analysis_long <- dat3 %>%
...
analysis_long <- analysis_long %>%
  mutate(
    race_grp = case_when(
      RACE_bl %in% c(1, 2) ~ "White",
      RACE_bl %in% c(3, 4) ~ "Black",
      RACE_bl %in% c(5, 6, 7, 8) ~ "Other",
      TRUE ~ NA_character_
    ),
    educ_grp = case_when(
