...
```{r tbl1, echo = F, results='asis', include = FALSE}
...
# Return named list of plots instead of printing directly
  list(
    trace_no       = mcmc_trace(draws_no,        pars = pars_no)  + ggtitle(paste(outcome_label, "- Trace (No adherence)")),
    dens_no        = mcmc_dens_overlay(draws_no,  pars = pars_no)  + ggtitle(paste(outcome_label, "- Density (No adherence)")),
    trace_adh      = mcmc_trace(draws_adh,        pars = pars_adh) + ggtitle(paste(outcome_label, "- Trace (With adherence)")),
    dens_adh       = mcmc_dens_overlay(draws_adh, pars = pars_adh) + ggtitle(paste(outcome_label, "- Density (With adherence)")),
    acf_adh        = mcmc_acf(draws_adh,          pars = pars_adh) + ggtitle(paste(outcome_label, "- ACF (With adherence)"))
  )
}
...
```{r Fig1, echo = F, include=FALSE}
###############################################
## Making Comparison tables and graphs for Report
###############################################
...
```{r tbl2 , echo = F, include=FALSE}
# Table 2a: Hard drug use effect - No adherence
make_display_table(
  combined_results %>% filter(Model == "No adherence"),
  filter_term  = "Hard drug use (Yes vs No)",
  caption_text = "Table 2a. Effect of baseline hard drug use on Year 2 outcomes - model without adherence adjustment.Clinically meaningful thresholds: 0.5 log copies/mL (Viral load), 50 cells/uL (CD4), 2 points (QOL)."
)
...
make_display_table(
  ...
  filter_term  = "Hard drug use (Yes vs No)",
  caption_text = "Table 2b. Effect of baseline hard drug use on Year 2 outcomes - model adjusted for adherence.Clinically meaningful thresholds: 0.5 log copies/mL (Viral load), 50 cells/uL (CD4), 2 points (QOL)."
)

```


```{r tbl3, echo = F, include=FALSE}
# Table 2c: Adherence effect - With adherence
make_display_table(
  combined_results %>% filter(Model == "With adherence"),
  filter_term  = "Adherence ≥95% (vs <95%)",
  caption_text = "Table 2c. Effect of adherence (≥95% vs <95%) on Year 2 outcomes - adherence-adjusted model.Clinically meaningful thresholds: 0.5 log copies/mL (Viral load), 50 cells/uL (CD4), 2 points (QOL)."
)
```
...
```{r tbl4, echo = F, include=FALSE}
#################################################
## LOO comparison withing each outcome
################################################
...
<!-- Title -->
**Impact of Baseline Hard Drug Use on HAART Treatment Response: A Frequentist and Bayesian Analysis of the MACS Cohort**
...
### Introduction
Data for this project were obtained from the Multicenter AIDS Cohort Study (MACS), a prospective cohort study of HIV-1 infection in homosexual and bisexual men followed annually across four U.S. cities after HAART initiation. Year 0 represents each participant's last untreated visit immediately prior to starting HAART and all subsequent visits are on-treatment. The primary exposure is self-reported hard drug use (such as heroin or cocaine) at baseline. Treatment response at Year 2 was evaluated using four outcomes: viral load (log10 copies/mL), CD4+ T-cell count (cells/uL), physical quality of life score (AGG_PHYS), and mental quality of life score (AGG_MENT), both on the SF-36 scale (0–100).

The scientific hypothesis is that baseline hard drug use is associated with poorer treatment response two years after HAART initiation. To evaluate this, for each outcome we compared Year 2 responses between baseline hard drug users and non-users while adjusting for the corresponding baseline outcome value and baseline covariates. For each outcome, the primary statistical hypotheses were:

$H_0$: There is no difference in Year 2 outcome between baseline hard drug users and non-users ($\beta_1 = 0$),

$H_1$: Baseline hard drug use is associated with the Year 2 outcome ($\beta_1 \neq 0$)

where $\beta_1$ is the adjusted coefficient for hard drug use in an ANCOVA model for the Year 2 outcome. A secondary analysis adjusts for Year 2 medication adherence to assess whether any observed difference is explained by differential adherence between groups.

### Methods

**Data Management:**
The analytic population included HIV-positive participants with complete baseline (Year 0) visit and a Year 2 follow-up visit, and who had ever initiated antiretroviral therapy (everART = 1). Viral load (VLOAD) was log10-transformed to reduce skew and stabilize variance. BMI values coded as 999 or -1 as well as biologically implausible values were set to to missing. Race was re-categorized into White Non Hispanic and Other, education into high school or less, some college, and college or higher, smoking status into current versus not current. Year 2 adherence (ADH) was dichotomized as $\geq$95% versus <95%. The primary exposure baseline hard drug use was defined from the Year 0 visit. All covariates (age, BMI, race, education, smoking status) were fixed at baseline. Data were restructured to wide format pairing Year 0 and Year 2 values per participant. Participants missing the exposure, any covariate, or any outcome were excluded
from the analytic dataset via complete-case analysis.

**Descriptive Statistics:**
Baseline characteristics of participants were summarized by baseline hard drug use status in Table 1. Mean outcome trajectories from Year 0 to Year 2 with 95% confidence bands were plotted by group with individual trajectories for a subsample of 60 participants (Figure 1).

**Frequentist Approach:**
For each of the four outcomes, an ANCOVA framework will be used: $Y_{2} = \beta_0 + \beta_1(\text{Hard Drug Use}_{baseline}) + \beta_2 Y_{0} + \mathbf{X}\beta + \varepsilon,$ where $Y_2$ = Year 2 outcome, $Y_0$ = Baseline value of the outcome, $X$ = baseline covariates (age, BMI, race, education, and smoking status). $\beta_1$ represents the adjusted difference in Year 2 outcome between baseline hard drug users and non-users, and $\varepsilon_i \sim \mathcal{N}(0, \sigma^2)$. This approach improves efficiency and controls for baseline outcome differences. This approach estimates the adjusted mean difference in Year 2 outcomes between hard drug users and non-users, conditional on pre-treatment values and potential confounders.

A second set of models was fit adding Year 2 adherence ($\geq95\%$ vs. $<95\%$) as a covariate. Comparing the hard drug use coefficient between the unadjusted and adherence-adjusted models allowed assessment of whether differential treatment adherence between groups explained any observed association between baseline drug use and treatment response. The hard drug coefficient was tested at $\alpha = 0$ with no multiple comparisons correction. Point estimates are reported with 95% confidence intervals.

**Bayesian Analysis:**
Each of the eight ANCOVA models was also fit using Bayesian linear regression implemented in Stan via the cmdstanr interface. The model structure was identical to the frequentist models. Weakly informative priors were placed on all regression coefficients: $\beta \sim Normal(0, 100^2)$. A Half-Normal(0, 100) prior was specified for $\sigma$. Posterior distributions were estimated using the Hamiltonian Monte Carlo (HMC) algorithm and its adaptive variant, the No-U-Turn Sampler (NUTS), with four independent chains, each run for 1,000 warm-up and 2,000 sampling iterations. Convergence was confirmed via R-hat < 1.01 and bulk/tail ESS > 400 (Appendix Table 5). Trace plots, posterior density overlays, and autocorrelation plots demonstrated satisfactory mixing and efficient sampling for all model parameters (Appendix Figures 3–6).


For each outcome, the posterior distribution of the hard drug use coefficient was summarized by posterior mean and 95% highest posterior density interval (HPDI). The posterior probability that the coefficient was negative indicating worse outcomes for hard drug users was computed as P($\beta < 0$). To address clinical relevance beyond statistical significance, the posterior probability that the effect exceeded a pre-specified clinically meaningful threshold was computed as $P(|\beta| >$ threshold), with thresholds of 0.5 log10 copies/mL for viral load, 50 cells/uL for CD4+ T-cell count, and 2 SF-36 points for the QOL outcomes. Model fit between the unadjusted and adherence-adjusted models was compared using Leave-One-Out Information Criterion (LOO-IC). Lower LOO-IC values indicate better predictive accuracy.

All analyses were conducted in R version 4.3.1 (2023-06-16 ucrt), with Bayesian models implemented in Stan via the cmdstanr interface.

### Results
**Descriptive statistics (Table 1):**
The analytic cohort included 463 participants, 36 ($7.8\%$) reported baseline hard drug use. The baseline characteristics were largely similar between groups. Hard drug users were more likely to be current smokers ($75.0\%$ vs $34.9\%$) and had slightly lower BMI, while baseline viral load, CD4+ T-cell count, and quality-of-life measures were comparable. At Year 2, the majority of participants reported $\geq95\%$ adherence. Trajectory plots (Figure 1) showed expected treatment response after HAART initiation: declining viral load and increasing CD4+ T-cell counts in both groups, with stable QOL scores over two years.

**Hard Drug Use Effect:**
Frequentist and Bayesian ANCOVA results for the effect of baseline hard drug use on Year 2 outcomes are presented in Table 2. After adjusting for baseline outcomes and covariates, hard drug users showed higher viral load, lower CD4+ T-cell count, and lower QOL scores at Year 2 compared to non-users. Estimates were consistent between unadjusted and adherence-adjusted models, suggesting that differential adherence did not fully explain the associations. Bayesian posterior means and 95% HPDIs were closely aligned with the frequentist estimates across all outcomes, as expected given the weakly informative priors, confirming robustness across inferential frameworks. Figure 2 shows that for CD4 count, posterior mass is concentrated below zero and past the 50-cell threshold, while viral load and QOL posteriors remain near zero with greater uncertainty.

**Adherence Effect:**
Table 3 presents the effect of medication adherence ($\geq95$% vs. $<95$%) on Year 2 outcomes from the adherence-adjusted models. Higher adherence ($\geq$95%)  was significantly associated with lower viral load ($\beta$ = -0.44, 95% CI: -0.79, -0.09; p = 0.014), with the Bayesian estimate closely agreeing (posterior mean = -0.44, 95% HPDI: -0.78, -0.08). Higher adherence was also associated with significantly greater CD4+ count ($\beta$ = 55.54, 95% CI: 0.49, 110.58; p = 0.048; Bayesian mean = 54.51, 95% HPDI: 1.92, 106.29). Associations with physical and mental QOL were positive but non-significant ($\beta$ = 2.13, 95% CI: -0.26, 4.52; p = 0.081 for physical QOL and $\beta$ = 2.30, p = 0.139 for mental QOL). The posterior probability of a clinically meaningful adherence effect exceeded the pre-specified thresholds for viral load and CD4 outcomes, supporting the clinical relevance of adherence.

**Model Fit:**
Table 4 presents LOO-IC comparisons between unadjusted and adherence-adjusted Bayesian models. The adherence-adjusted model showed modestly better fit for viral load ($\Delta$LOO-IC = -3.2), CD4 count ($\Delta$LOO-IC = -2.6), and physical QOL ($\Delta$LOO-IC = -1.7). For mental QOL, the unadjusted model performed marginally better ($\Delta$LOO-IC = +0.2). However, none of the differences exceeded the commonly used threshold of four LOO-IC units.

### Discussion
This analysis examined whether baseline hard drug use was associated with HAART treatment response at Year 2 across laboratory and quality-of-life outcomes in HIV-positive men from the MACS cohort. The association was most pronounced for CD4+ T-cell count and persisted after adherence adjustment, indicating adherence only partially explain the immune suppression observed among hard drug users. Associations with viral load and quality of life outcomes were smaller and less precisely estimated. Medication adherence emerged as an independent predictor of improved laboratory outcomes, reinforcing its central role in HIV treatment and suggesting that adherence support may partially offset the adverse effects of substance use. Frequentist and Bayesian analyses yielded consistent results across all models. The Bayesian framework additionally quantified the probability of clinically meaningful effects and this information unavailable from p-values alone. LOO-IC comparisons showed modest improvements with adherence adjustment for laboratory outcomes, with negligible gain for QOL outcomes.

Limitations include a small hard drug user group (n = 39) limiting precision, potential selection bias from complete-case analysis, and measurement of drug use only at baseline which may not reflect continued or changing use over follow-up. Despite these limitations, the findings highlight the importance of addressing substance use and adherence concurrently in HIV care to optimize immune recovery following HAART initiation.
...
tbl1 <- table1(
  ~ age_bl + BMI_bl + race_grp + educ_grp + SMOKE_bl + logVLOAD + LEU3N +
    AGG_PHYS + AGG_MENT + adh_cat | hard_drugs_bl,
  data = tab1_dat,
  caption = "Baseline characteristics (and selected Year 2 follow-up measures) of participants by baseline hard drug use status."
)
...
```{r, echo = F, fig.width=8, fig.height=6, fig.cap="Mean outcome trajectories from Year 0 to Year 2 by baseline hard drug use status. Bold lines show group means with 95 percent confidence bands; faded lines show individual trajectories for a random subsample of 60 participants."}
...
make_combined_hardrug_table <- function(data, caption_text) {
  ...
  data %>%
    ...
    mutate(
      ...
      `Bayes: P(b<0)`      = sprintf("%.3f", bayes_pr_lt0),
    ...
    select(Outcome, Model,
           `Freq: Est (95% CI)`, `Freq: p`,
           `Bayes: Mean (95% HPDI)`, `Bayes: P(b<0)`, `Bayes: P(Clin)`) %>%
    kable(
      format    = "latex",
      caption   = caption_text,
      align     = "llccccc",
    ...
    footnote(
      general = paste(
        "Clin. thresholds: Viral load 0.5 log10 copies/mL;",
        "CD4 50 cells/uL; QOL 2 points."
      ),
      symbol = c(
        "P(b<0) = posterior probability effect is negative.",
        "P(Clin.) = posterior probability effect exceeds clinical threshold."
      ),
      general_title     = "Note: ",
...
make_combined_hardrug_table(
  combined_results,
  caption_text = "Effect of baseline hard drug use on Year 2 outcomes, unadjusted and adjusted for adherence."
)
```


```{r, echo = F, results='asis'}
# Table 3: Adherence effect - With adherence

make_display_table2 <- function(data, filter_term, caption_text, add_clin_col = TRUE) {
...
make_display_table2(
  combined_results %>% 
    filter(Model == "With adherence") %>%
    mutate(Term = gsub("≥", ">=", gsub("%", " percent", Term))),
  filter_term  = "Adherence >=95 percent (vs <95 percent)",
  caption_text = "Effect of adherence (95 percent or above vs below 95 percent) on Year 2 outcomes. Thresholds: 0.5 log10 copies/mL (Viral load), 50 cells/uL (CD4), 2 points (QOL)."
)
```
...
```{r, echo=F, results='asis'}
# Table 4: LOO comparison
loo_comparison %>%
  mutate(across(c(looic_no_adh, looic_with_adh, delta_looic), ~sprintf("%.1f", .))) %>%
  kable(
    caption   = "LOO-IC comparison: adherence-adjusted vs unadjusted models. Lower LOO-IC = better fit.",
    col.names = c("Outcome", "LOO-IC (No adherence)", 
                  "LOO-IC (With adherence)", "Delta LOO-IC"),
    align     = "lcccc",
    booktabs  = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position", "striped"), font_size = 10)
...
```{r, echo = F, inlcude = F, echo=FALSE, fig.width=10, fig.height=8, fig.cap="Posterior distributions of the hard drug use coefficient by outcome. Dashed line = no effect; dotted lines = clinical thresholds."}
...
p1 <- plot_posterior_density(fit_vload, fit_vload_adh, "Viral Load (log10)", 0.5, "Effect (log10 copies/mL)")
p2 <- plot_posterior_density(fit_cd4,   fit_cd4_adh,   "CD4 Count",          50,  "Effect (cells/uL)")
p3 <- plot_posterior_density(fit_phys,  fit_phys_adh,  "Physical QOL",        2,  "Effect (SF-36 points)")
p4 <- plot_posterior_density(fit_ment,  fit_ment_adh,  "Mental QOL",          2,  "Effect (SF-36 points)")
...
File name for Code: Project1/Code/Project 1 Report.rmd

File name for Final report:Project1/Reports/Project 1 Report.pdf

Directory for the raw dataset: C:/Users/mahim/OneDrive - The University of Colorado Denver/BIOS6624/Project1/DataRaw

Directory for the cleaned/analytic dataset: C:/Users/mahim/OneDrive - The University of Colorado Denver/BIOS6624/Project1/DataProcessed
...
diag_conv %>%
  ...
  kable(
    caption   = "MCMC convergence diagnostics for all Bayesian models.",
    col.names = c("Outcome", "Model", "Max R-hat", 
                  "Min Bulk ESS", "Min Tail ESS", "Converged?"),
    align     = "llcccc",
    booktabs  = TRUE
...
diag_fit %>%
  ...
  kable(
    caption   = "LOO-IC model fit statistics for all Bayesian models.",
...
```{r figA1, echo=FALSE, fig.width=12, fig.height=10, fig.cap="MCMC diagnostics for Viral Load model. Top row: unadjusted model. Bottom rows: adherence-adjusted model."}
p <- plot_mcmc_diag(fit_vload, fit_vload_adh, "Viral Load")
ggarrange(
  p$trace_no, p$dens_no,
  p$trace_adh, p$dens_adh, p$acf_adh,
  ncol = 2, nrow = 3
)
```

```{r figA2, echo=FALSE, fig.width=12, fig.height=10, fig.cap="MCMC diagnostics for CD4 Count model."}
p <- plot_mcmc_diag(fit_cd4, fit_cd4_adh, "CD4 Count")
ggarrange(
  p$trace_no, p$dens_no,
  p$trace_adh, p$dens_adh, p$acf_adh,
  ncol = 2, nrow = 3
...
```{r figA3, echo=FALSE, fig.width=12, fig.height=10, fig.cap="MCMC diagnostics for Physical QOL model."}
p <- plot_mcmc_diag(fit_phys, fit_phys_adh, "Physical QOL")
ggarrange(
  p$trace_no, p$dens_no,
  p$trace_adh, p$dens_adh, p$acf_adh,
  ncol = 2, nrow = 3
...
```{r figA4, echo=FALSE, fig.width=12, fig.height=10, fig.cap=" MCMC diagnostics for Mental QOL model."}
p <- plot_mcmc_diag(fit_ment, fit_ment_adh, "Mental QOL")
ggarrange(
  p$trace_no, p$dens_no,
  p$trace_adh, p$dens_adh, p$acf_adh,
  ncol = 2, nrow = 3
